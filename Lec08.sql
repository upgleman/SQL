...
-- REQUEST 테이블에는 요청 ID와 요청상태 정보가 담겨있습니다. 
...
-- REQUEST_DTL 테이블에서 해당 요청 ID에 대한 상세정보(요청 상세 ID(request_dtl_id), 
-- 요청 ID(request_id), 요청 이름(request_name), 요청 내용(request_content))을 출력하는 쿼리를 작성해봅시다.
...
-- 가게 이름 정보는 STORE 테이블에 있기때문에 다중 행 서브쿼리 EXISTS를 이용하여 PAYMENT 테이블에서 결제금액이 5,000원 이상이고 
-- STORE 테이블의 가게 ID와 일치하는 데이터가 존재하는 경우의 가게 ID와 가게 이름을 조회해 봅시다.
-- 두 개의 테이블을 이용하여, 결제금액이 5,000원 이상인 가게에 대해서, 가게 ID와 가게 이름을 조회하는 쿼리를 작성해봅시다.
-- 두 개의 테이블을 이용하여, 결제금액이 5,000원 이상인 가게에 대해서, 가게 ID와 가게 이름을 조회하는 쿼리를 작성해봅시다.
-- STORE 테이블을 기준으로 조회하면서 EXISTS문을 응용하여 서브쿼리상에서, PAYMENT 테이블에 대해 STORE 테이블의 가게 ID(store_id)와 일치하고 
-- 결제금액이 5,000원 이상인 데이터가 존재하는지를 통해 가게 ID(store_id), 가게 이름(store_name)을 조회하는 쿼리를 작성해보세요.
...
-- 직원들 중에서 모든 사원의 급여보다 급여가 높거나 같은 직원을 조회하려고 합니다. 앞에서 배운 ALL을 이용하여, 
-- 모든 사원의 급여보다 크거나 같은 급여에 대한 조건을 작성해 봅시다.
...
-- EMPLOYEE 테이블에 대해, 1번에서 완성한 쿼리와 ALL을 이용하여, 
-- 모든 사원들의 급여보다 높거나 같은 급여 를 가진 직원의 직원 ID(employee_id), 급여(salary), 직급(position) 을 조회하는 쿼리를 작성해봅시다.
...
-- 음식 중에서 임의의 Main 음식보다 가격이 비싼 음식을 조회하고자 합니다. 
-- 앞에서 배운 ANY를 이용하여 음식의 가격이 임의의 Main 음식보다 가격이 비싼지에 대한 조건을 작성해 봅시다.

-- 음식 중에서 음식 타입이 Main인 음식들의 가격을 구한 뒤, ANY를 이용하여 임의의 Main 음식보다 가격이 비싼 음식들에 대해서 조회하는 쿼리를 작성해 봅시다.

-- FOOD테이블에서 음식 타입(food_type)이 Main인 임의(ANY)의 음식 가격(price) 보다 가격이 비싼 음식들의 음식 ID(food_id), 
-- 음식 타입(food_type), 음식 이름(food_name), 가격(price) 을 조회하는 쿼리를 작성해봅시다.
